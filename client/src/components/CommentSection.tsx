 ...
 * Supports inline mode (just the toggle button) for action row placement
 */
...
export default function CommentSection({ pointId, layout = "left", inline = false }: CommentSectionProps) {
  const { lang } = useLang();
  ...
  // In inline mode: render toggle button inline, and the panel expands below the action row
  if (inline) {
    return (
      <>
        {toggleButton}
  ...
  // Standard mode: full section with toggle + panel
  return (
